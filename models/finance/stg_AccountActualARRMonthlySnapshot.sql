with Account_ARR_MonthlySnapshots as (        
...
from
 (select D.Date,account_id,SUBSCRIPTION_STATUS_C  AccountStatus,SUBSCRIPTION_END_DATE_C  End_Date, SUBSCRIPTION_START_DATE_C  Start_Date, Close_Date, Stage_Name, Total_ARR_C,type 
  from SALESFORCE2.Opportunity o 
  left join
  (select distinct DATE from SALESFORCE2.Dim_Date where DAY(DATE) = 1
 ...
 a.Type, ams.DESCRANK
  ...
  join SALESFORCE2.Account a on a.ID = ams.Account_ID
  where Date IS NOT null